...
sequenceDiagram
    ...
    alt 問題ない場合
        main ->> Person B : 最新のブランチの内容を自分のローカルに反映
        Note over Person B : (問題ない場合) 最新のブランチの内容を自分のローカルに反映
        Note over Person B : git fetch
        Note over Person B : git merge origin/main
